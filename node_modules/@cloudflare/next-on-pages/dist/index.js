...
// ../../node_modules/zodcli/dist/index.js
var booleanArgument = z.union([
  z.literal("true").transform(() => true),
  z.literal("false").transform(() => false),
  z.null().transform(() => true)
]);
var argumentParser = ({ options, aliases }) => {
    ...
    for (const key in rawResults) {
      ...
      if (!isSupposedToBeAnArray && rawResults[key].length > 1) {
        ctx.addIssue({
          ...
          message: `Cannot parse argument: \`${key}\`. Unexpected multiple values: ${rawResults[key].map((value) => `\`${value}\``).join(", ")}.`
        });
        continue;
...
function hasFlag(flag2, argv = globalThis.Deno ? globalThis.Deno.args : import_node_process.default.argv) {
...
async function validatePathType(path, type2) {
  try {
    const stats = await (0, import_promises.stat)(path);
    if (type2 === "file" && stats.isFile())
      return true;
    if (type2 === "directory" && stats.isDirectory())
      return true;
  } catch (e) {
  }
  return false;
  ...
  try {
    ...
    const paths = await Promise.all(
      files.map(async (file) => {
        const path = (0, import_node_path.resolve)(dir, file);
...
async function copyFileWithDir(sourceFile, destFile) {
  await (0, import_promises.mkdir)((0, import_node_path.dirname)(destFile), { recursive: true });
  await (0, import_promises.copyFile)(sourceFile, destFile);
}
async function readDirectories(basePath) {
  try {
    ...
    const dirs = await Promise.all(
      files.map(async (file) => {
        const path = normalizePath((0, import_node_path.join)(basePath, file.name));
        const isSymbolicLink = file.isSymbolicLink();
        const isDirectory = file.isDirectory() || isSymbolicLink && await validateDir(path);
...
function stripIndexRoute(path) {
  return addLeadingSlash(path.replace(/\/index$/, ""));
}
function addLeadingSlash(path) {
  return path.startsWith("/") ? path : `/${path}`;
}
function stripFuncExtension(path) {
...
function getRouteOverrides(routePath) {
  ...
  return [
    .../* @__PURE__ */ new Set([formattedPathName, withoutHtmlExt, strippedIndexRoute])
...
// src/buildApplication/packageManagerUtils.ts
...
async function getCurrentPackageManager() {
  ...
  if (hasPnpmAgent || hasPnpmLock)
    return "pnpm";
  if (hasYarnAgent || hasYarnLock) {
    const { baseCmd } = await getPackageManagerInfo("yarn");
    const getYarnV = (0, import_child_process.spawn)(baseCmd, ["-v"]);
    let yarnV = "";
    getYarnV.stdout.on("data", (data2) => {
      yarnV = `${data2}`.trimEnd();
    });
    getYarnV.stderr.on("data", (data2) => cliError(`
${data2}`));
    await waitForProcessToClose(getYarnV);
    if (!yarnV.startsWith("1.")) {
      const yarnrc = await (0, import_promises2.readFile)(".yarnrc.yml", "utf-8");
      const { nodeLinker } = js_yaml_default.load(yarnrc);
...
async function getPackageManagerInfo(pm) {
  ...
  switch (pm) {
    case "bun":
      return {
        ...
        execArgs: ["x"],
        infoArgs: ["pm", "ls"],
        getPackageVersionRegex: (name) => new RegExp(`^.+ ${name}@(.*)$`, "im")
      };
    case "pnpm":
      return {
        pm,
        baseCmd: `pnpm${cmd}`,
        dlxOrExec: (useDlx) => [useDlx ? "dlx" : "exec"],
    ...
    case "yarn":
      return {
        pm,
        baseCmd: `yarn${cmd}`,
        dlxOrExec: pm === "yarn (berry)" ? (useDlx) => [useDlx ? "dlx" : "exec"] : void 0,
        infoArgs: ["info"]
      };
    case "npm":
    default:
...
async function getPackageVersion(packageName, packageManager) {
  try {
    packageManager ??= await getCurrentPackageManager();
    const { pm, baseCmd, infoArgs, getPackageVersionRegex } = await getPackageManagerInfo(packageManager);
    const commandOutput = (0, import_child_process.execFileSync)(
      baseCmd,
    ...
    if (getPackageVersionRegex) {
      ...
      const packageInfo = pm === "pnpm" ? commandOutputJson[0] : commandOutputJson;
      packageVersion = pm.startsWith("yarn") ? packageInfo?.children?.Version : packageInfo?.dependencies?.[packageName]?.version ?? packageInfo?.devDependencies?.[packageName]?.version;
    }
...
async function getBinaryVersion(binaryName) {
  const versionFromProcess = process.versions[binaryName];
  if (versionFromProcess)
    return versionFromProcess;
  try {
    const cmd = isWindows() ? ".cmd" : "";
    return (0, import_child_process.execFileSync)(`${binaryName}${cmd}`, ["--version"]).toString().trim().replace(/^v/, "");
  } catch {
    return null;
  }
}
...
async function getExecStr(pm, packageName) {
  const {
    baseCmd,
    execCmd,
    dlxOrExec,
...
var cliOptions = z.object({
  ...
  noColor: flag,
  info: flag,
  outdir: z.string().optional().default((0, import_path.join)(".vercel", "output", "static")).transform((path) => normalizePath((0, import_path.resolve)(path))).refine(
    (path) => {
      const currentWorkingDirectory = normalizePath((0, import_path.resolve)());
      return path.startsWith(currentWorkingDirectory) && path !== currentWorkingDirectory;
    },
    {
      message: "The output directory should be inside the current working directory"
    }
  )
}).strict();
function parseCliError(error) {
  if (error instanceof z.ZodError && error.issues.length > 0) {
    ...
    if ((issue.code === "custom" || issue.code === "invalid_type") && issue.path.length) {
...
async function printEnvInfo() {
  ...
  const [nodeVersion, bunVersion, pnpmVersion, yarnVersion, npmVersion] = await Promise.all(
...
async function getVercelConfig() {
  const configPath = (0, import_path2.join)(".vercel", "output", "config.json");
...
function constructBuildOutputRecord(item, outputDir) {
  ...
  return `{
				type: ${JSON.stringify(item.type)},
				entrypoint: '${item.entrypoint.replace(outputDir, "").replace(
    /^\/_worker\.js\/__next-on-pages-dist__\//,
    "./__next-on-pages-dist__/"
  )}'
			}`;
}
async function buildWorkerFile({ vercelConfig, vercelOutput }, outputDir, minify) {
  const functionsFile = (0, import_path3.join)(
    (0, import_os2.tmpdir)(),
  ...
  const outputFile = (0, import_path3.join)(outputDir, "_worker.js", "index.js");
  await (0, import_esbuild.build)({
    entryPoints: [(0, import_path3.join)(__dirname, "..", "templates", "_worker.js")],
    banner: {
      js: generateGlobalJs()
    },
    bundle: true,
  ...
  return (0, import_path3.relative)(".", outputFile);
}
...
async function generateProjectJsonFileIfNeeded() {
  const projectJsonFilePath = (0, import_path4.join)(".vercel", "project.json");
...
async function createTempVercelConfig(config) {
  const oldConfigPath = (0, import_path4.join)("vercel.json");
  const originalConfig = await readJsonFile(oldConfigPath);
  const tempConfigPath = (0, import_path4.join)(".vercel", "temp-nop-config.json");
  const tempConfig = {
    framework: "nextjs",
    ...config,
    ...originalConfig
...
async function runVercelBuild(pkgMng, additionalArgs = []) {
  const { pm, baseCmd } = await getPackageManagerInfo(pkgMng);
  if (pm === "yarn (classic)") {
    cliLog(
      `Installing vercel as dev dependencies with '${baseCmd} add vercel -D'...`
    );
    const installVercel = (0, import_child_process2.spawn)(baseCmd, ["add", "vercel", "-D"]);
...
async function getVercelBuildChildProcess(pkgMng, additionalArgs = []) {
  const { pm, baseCmd, execCmd, execArgs, dlxOrExec } = await getPackageManagerInfo(pkgMng);
  let dlxArgs = [];
  if (dlxOrExec) {
    const vercelPackageIsInstalled = await getPackageVersion("vercel", pm);
    dlxArgs = dlxOrExec(!vercelPackageIsInstalled);
  }
  return (0, import_child_process2.spawn)(execCmd ?? baseCmd, [
    ...dlxArgs,
    ...execArgs ?? [],
    "vercel",
    "build",
...
async function deleteNextTelemetryFiles(outputDir) {
  const nextDir = (0, import_path4.resolve)(outputDir, "_next");
  const privateDir = (0, import_path4.join)(nextDir, "__private");
...
async function buildNextStaticHeaders(outputDir) {
  ...
  if (nextStaticHeaders) {
    await (0, import_promises5.writeFile)(
      (0, import_path5.join)(outputDir, "_headers"),
      `
...
async function buildRoutes(outputDir, opts) {
  ...
  try {
    await (0, import_promises5.writeFile)(
      (0, import_path5.join)(outputDir, "_routes.json"),
      JSON.stringify({
        version: 1,
...
async function getVercelStaticAssets() {
  const dir = (0, import_path6.resolve)(".vercel", "output", "static");
  if (!await validateDir(dir)) {
    cliLog("No static assets detected.");
    return [];
  }
  await deleteNextTelemetryFiles(dir);
  return (await readPathsRecursively(dir)).map((file) => addLeadingSlash(normalizePath((0, import_path6.relative)(dir, file)))).filter(
    (path) => !/^\/_worker\.js\//.test(path) && !["/_headers", "/_routes.json"].includes(path)
  );
}
async function copyVercelStaticAssets(vercelDir, outputDir, staticAssets) {
  ...
  await Promise.all(
    staticAssets.map(async (file) => {
      const src = (0, import_path6.join)(vercelDir, file);
      const dest = (0, import_path6.join)(outputDir, file);
      try {
        await (0, import_promises6.mkdir)((0, import_path6.dirname)(dest), { recursive: true });
...
async function processOutputDir(outputDir, staticAssets) {
  const vercelDir = normalizePath((0, import_path6.resolve)(".vercel", "output", "static"));
  if (outputDir !== vercelDir) {
    cliLog(`Using custom output directory: ${(0, import_path6.relative)(".", outputDir)}`);
    ...
    await (0, import_promises6.rm)((0, import_path6.join)(outputDir, "_worker.js"), { recursive: true, force: true });
  }
}
...
function printBuildSummary(staticAssets, { vercelOutput }, {
  ...
  const middlewareFunctions = [...vercelOutput.entries()].filter(([, { type: type2 }]) => type2 === "middleware").map(([path]) => path).sort((a, b) => a.localeCompare(b));
  ...
  const otherStatic = staticAssets.filter((path) => !prerenderedPaths.has(path)).sort((a, b) => {
...
async function writeBuildInfo(outputDir, staticAssets, { vercelOutput }, {
  collectedFunctions,
  identifiers
} = emptyProcessedVercelFunctions) {
  const currentDir = (0, import_node_path2.resolve)();
  const buildLogFilePath = (0, import_node_path2.join)(outputDir, "nop-build-log.json");
  const {
    edgeFunctions,
  ...
  const buildLogObject = {
    timestamp: Date.now(),
    outputDir: (0, import_node_path2.relative)(currentDir, outputDir),
    versions: {
      "@cloudflare/next-on-pages": nextOnPagesVersion
    },
    buildFiles: {
      functions: {
        ...
        middleware: [...vercelOutput].filter(([, { type: type2 }]) => type2 === "middleware").map(([path]) => path),
  ...
  cliLog(`Build log saved to '${(0, import_node_path2.relative)(currentDir, buildLogFilePath)}'`);
}
function formatIdentifiersMap(identifiersMap) {
...
function constructSummarySections(sections) {
  return sections.map(({ name, rawItems, limit: limit2 }) => {
    ...
    const formattedItems = items.map((path, idx) => `  ${getItemPrefix(items.length, idx)} ${path}`).join("\n");
...
async function collectFunctionConfigsRecursively(baseDir, configs = {
  ...
  for (const { path } of dirs) {
    if (path.endsWith(".func")) {
      const configPath = (0, import_node_path3.join)(path, ".vc-config.json");
      const config = await readJsonFile(configPath);
      const relativePath = addLeadingSlash(
        normalizePath((0, import_node_path3.relative)(configs.functionsDir, path))
      );
...
async function checkEntrypoint(fullPath, relativePath, entrypoint) {
  ...
  if (!await validateFile((0, import_node_path4.join)(fullPath, finalEntrypoint))) {
    if (isMiddleware) {
      cliWarn(
...
function replaceRscWithNonRsc({ edgeFunctions, invalidFunctions, ignoredFunctions }, tempFunctionsMap, { formattedPath, path }) {
...
async function getRoutePath({ fallback }, relativePath, { functionsDir: functionsDir2 }) {
  const prerenderRoute = (0, import_node_path5.join)((0, import_node_path5.dirname)(relativePath), fallback.fsPath);
  const prerenderFile = (0, import_node_path5.join)(functionsDir2, prerenderRoute);
...
async function getRouteDest({ fallback }, relativePath, { outputDir }) {
  ...
  const destRoute = normalizePath((0, import_node_path5.join)((0, import_node_path5.dirname)(relativePath), fixedFileName));
  const destFile = (0, import_node_path5.join)(outputDir, destRoute);
...
function collectIdentifiersByType({ foundIdentifiers, programIdentifiers }, { program, entrypoint }, callback) {
  const rawIdentifiers = program.body.map(callback).flat().filter(Boolean);
...
function groupIdentifiersByType(foundIdentifiers) {
  ...
  for (const [identifier, info] of foundIdentifiers) {
    ...
    if (!existing) {
      groupedIdentifiers.set(key, /* @__PURE__ */ new Set([identifier]));
    } else {
...
async function buildFile(contents, filePath, { relativeTo } = {}) {
  ...
  await (0, import_promises8.mkdir)((0, import_node_path6.dirname)(filePath), { recursive: true });
...
var functionsDir = (0, import_node_path6.resolve)(".vercel", "output", "functions");
function getFunctionNestingLevel({
  ...
  let nestingLevel = -1;
  try {
    const relativePath = (0, import_node_path6.relative)(from, relativeTo);
...
var nodeBuiltInModulesPlugin = {
  ...
  setup(build3) {
    build3.onResolve({ filter: /^node:/ }, ({ kind, path }) => {
      return kind === "require-call" ? { path, namespace: "node-built-in-modules" } : void 0;
    });
    build3.onLoad(
...
async function processFunctionIdentifiers({ edgeFunctions }, { entrypointsMap, identifierMaps }, opts) {
  ...
  for (const [path, fnInfo] of edgeFunctions) {
    ...
    const identifiers = entrypointsMap.get(entrypoint).sort((a, b) => b.start - a.start);
    const identifierPathsToBuild = /* @__PURE__ */ new Set();
    const importsToPrepend = [];
    const wasmImportsToPrepend = /* @__PURE__ */ new Map();
    const newFnLocation = (0, import_node_path7.join)("functions", `${fnInfo.relativePath}.js`);
    const newFnPath = (0, import_node_path7.join)(opts.nopDistDir, newFnLocation);
    for (const { type: type2, identifier, start, end, importPath } of identifiers) {
      const identifierInfo = identifierMaps[type2].get(identifier);
      if (importPath) {
        const { updatedContents } = await processImportIdentifier(
          { type: type2, identifier, start, end, importPath, info: identifierInfo },
        ...
        const { updatedContents, newFilePath, newImport, wasmImports } = await processCodeBlockIdentifier(
          { type: type2, identifier, start, end, info: identifierInfo },
    ...
    await Promise.all(
      [...identifierPathsToBuild].map(
        async (path2) => buildFile(await (0, import_promises9.readFile)(path2, "utf8"), path2)
      )
    );
...
async function buildFunctionFile({ fnInfo, fileContents, newFnLocation, newFnPath }, { importsToPrepend }, { workerJsDir, nopDistDir }) {
  ...
  groupedImports.forEach((keys, path) => {
    ...
    const importPath = normalizePath(
      (0, import_node_path7.join)(relativeImportPath, addLeadingSlash(path))
    );
    functionImports += `import { ${keys} } from '${importPath}';
`;
  });
  fnInfo.outputPath = (0, import_node_path7.relative)(workerJsDir, newFnPath);
  ...
  await Promise.all(
    [...wasmImportsToPrepend.entries()].map(
      async ([codeBlockRelativePath, wasmImports]) => {
        const filePath = (0, import_node_path7.join)(workerJsDir, codeBlockRelativePath);
        const relativeImportPath = getRelativePathToAncestor({
          from: filePath,
          relativeTo: nopDistDir
        });
        ...
        for (const identifier of wasmImports) {
          ...
          const wasmImportPath = normalizePath(
            (0, import_node_path7.join)(relativeImportPath, newDest)
          );
...
async function processImportIdentifier(ident, { fileContents, entrypoint, newFnLocation }, { nopDistDir, workerJsDir }) {
  const { type: type2, identifier, start, end, importPath, info } = ident;
  let updatedContents = fileContents;
  const codeBlock = updatedContents.slice(start, end);
  if (!info.newDest) {
    const importPathWithoutType = importPath.replace(/^(\.\.\/)+/, "").replace(new RegExp(`^/${type2}/`), "/");
    const oldPath = (0, import_node_path7.join)((0, import_node_path7.dirname)(entrypoint), importPath);
    const newPath = (0, import_node_path7.join)(nopDistDir, type2, importPathWithoutType);
    info.newDest = normalizePath((0, import_node_path7.relative)(workerJsDir, newPath));
  ...
  const newImportPath = normalizePath((0, import_node_path7.join)(relativeImportPath, info.newDest));
...
async function processCodeBlockIdentifier(ident, { fileContents, wasmIdentifierKeys }, { nopDistDir, workerJsDir }) {
  const { type: type2, identifier, start, end, info } = ident;
  let updatedContents = fileContents;
  const codeBlock = updatedContents.slice(start, end);
  ...
  } else if (type2 === "manifest") {
    newCodeBlock = `self.${identifier}=${identifier};`;
  }
  let newFilePath;
  const wasmImports = [];
  if (!info.newDest) {
    const identTypeDir = (0, import_node_path7.join)(nopDistDir, type2);
    newFilePath = (0, import_node_path7.join)(identTypeDir, `${info.groupedPath ?? identifier}.js`);
    info.newDest = normalizePath((0, import_node_path7.relative)(workerJsDir, newFilePath));
...
async function getFunctionFile(path, fnInfo) {
  const entrypoint = (0, import_node_path7.join)(path, fnInfo.config.entrypoint);
...
async function processBundledAssets({ edgeFunctions }, { nopDistDir }) {
  for (const [functionPath, { relativePath, config }] of edgeFunctions) {
    for (const { name, path } of config.assets ?? []) {
      const originalFile = (0, import_node_path7.join)(functionPath, path);
      const destFile = `${(0, import_node_path7.join)(nopDistDir, "assets", name)}.bin`;
      const relativeName = (0, import_node_path7.join)(relativePath, path);
...
async function checkForInvalidFunctions(collectedFunctions) {
...
async function printInvalidFunctionsErrorMessage(invalidFunctions) {
  const nextVersion = (0, import_semver.coerce)(await getPackageVersion("next"));
  const { exportText, exampleCode } = !nextVersion || (0, import_semver.gtr)(nextVersion, "13.1.2") ? {
...
async function prepareAndBuildWorker(outputDir, {
  ...
  const functionsDir2 = (0, import_path7.resolve)(".vercel", "output", "functions");
  if (!await validateDir(functionsDir2)) {
    cliLog(
      "No functions detected (no functions directory generated by Vercel)."
    );
  } else {
    const workerJsDir = (0, import_path7.join)(outputDir, "_worker.js");
    processedFunctions = await processVercelFunctions({
      functionsDir: functionsDir2,
      outputDir,
      workerJsDir,
      nopDistDir: (0, import_path7.join)(workerJsDir, "__next-on-pages-dist__"),
      disableChunksDedup
    });
  }
  ...
  await writeBuildInfo(
    (0, import_path7.join)(outputDir, "_worker.js"),
    staticAssets,
    processedVercelOutput,
    processedFunctions
  );
